...
export const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-xs font-semibold text-zinc-400 hover:text-cyan-400 transition-colors"
      >
      ...
      <GlassCard className="p-6" glow>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center text-zinc-950 font-bold shadow-lg shadow-cyan-500/20 shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-zinc-400 font-mono">{agent.badge}</span>
              </div>
              <h1 className="text-2xl font-bold font-display text-zinc-100 mt-1">{agent.name}</h1>
              <p className="text-xs text-zinc-400 mt-0.5">{agent.role}</p>
            </div>
          </div>
        </div>

        <p className="text-xs text-zinc-300 leading-relaxed mt-4 pt-4 border-t border-zinc-800">
          {agent.description}
        </p>

        {/* Capabilities Chips */}
        <div className="mt-4 pt-4 border-t border-zinc-800/80">
          <span className="text-[10px] font-mono uppercase text-zinc-400 font-semibold block mb-2">Agent Capabilities:</span>
          <div className="flex flex-wrap gap-2">
            {agent.capabilities.map((cap, i) => (
              <span key={i} className="px-3 py-1 rounded-xl bg-zinc-950/80 border border-zinc-800 text-[11px] text-cyan-300 flex items-center gap-1.5">
      ...
      <div>
        <span className="text-xs font-semibold text-zinc-400 font-mono uppercase tracking-wider block mb-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {agent.suggestedActions.map((act, i) => (
            <button
              ...
              className="p-3.5 rounded-2xl bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-800/80 text-left text-xs text-zinc-200 font-medium transition-all hover:border-cyan-500/40 flex items-center justify-between group"
            >
              <span>{act}</span>
      ...
      <GlassCard className="p-6 space-y-4">
        ...
        {/* Output Box */}
        {latestResult && (
          <div className="p-4 rounded-2xl bg-zinc-950 border border-cyan-500/30 text-xs text-zinc-200 leading-relaxed font-mono whitespace-pre-wrap max-h-80 overflow-y-auto">
            {latestResult}
          </div>
        )}
        ...
        >
          <input
            ...
            className="flex-1 bg-zinc-950 border border-zinc-800 focus:border-cyan-500 rounded-xl px-4 py-3 text-xs text-zinc-100 placeholder-zinc-500"
          />
          <button
            type="submit"
            disabled={isLoading || !prompt.trim()}
            className="px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-zinc-950 font-bold text-xs flex items-center gap-2 shadow-lg shadow-cyan-500/20"
          >
      ...
      <GlassCard className="p-6">
        <h3 className="text-sm font-bold text-zinc-100 mb-3 font-mono uppercase">
        ...
        {taskHistory.length === 0 ? (
          ...
          <div className="space-y-2 max-h-56 overflow-y-auto">
            {taskHistory.map(t => (
              <div key={t.id} className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800 text-xs flex items-center justify-between">
                <div>
                  <span className="font-semibold text-zinc-200">{t.task_type}</span>
