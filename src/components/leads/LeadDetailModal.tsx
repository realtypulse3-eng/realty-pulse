  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6 text-zinc-100">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300">
          <X className="w-5 h-5" />
        </button>

        <div className="flex items-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-500 to-cyan-500 flex items-center justify-center text-zinc-950 font-bold text-lg font-display shadow-lg shadow-blue-500/20">
            {lead.name.substring(0, 2).toUpperCase()}
          </div>
          <div>
            <h2 className="text-xl font-bold font-display text-zinc-100">{lead.name}</h2>
            <p className="text-xs text-zinc-400 font-mono">{lead.email} • {lead.phone}</p>
          </div>
        </div>

        {/* Qualification Badge Box */}
        <div className="p-4 rounded-xl bg-zinc-950/80 border border-zinc-800 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-[10px] font-mono uppercase text-zinc-400 block">AI Intent Qualification</span>
            <div className="flex items-center gap-2 mt-1">
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase font-mono ${
                lead.status === 'hot' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-zinc-800 text-zinc-300'
              }`}>
            ...
            {lead.qualification_reasoning && (
              <p className="text-xs text-zinc-300 mt-2 leading-relaxed">{lead.qualification_reasoning}</p>
            )}
          </div>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-zinc-950 font-semibold text-xs shadow-lg shadow-cyan-500/20 shrink-0"
          >
        ...
        <div className="grid grid-cols-2 gap-4 text-xs mb-6">
          <div className="p-3 bg-zinc-950/60 rounded-xl border border-zinc-800">
            <span className="text-zinc-400 block mb-1">Budget Range</span>
          ...
          <div className="p-3 bg-zinc-950/60 rounded-xl border border-zinc-800">
            ...
            <span className="text-sm font-bold text-zinc-100">{lead.preferred_location || 'Flexible'}</span>
          </div>
        </div>

        {/* AI Quick Actions */}
        <div className="p-4 rounded-xl bg-zinc-950/80 border border-blue-500/30 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs">
            <button
              ...
              className="p-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-200 font-medium flex items-center gap-2"
            >
